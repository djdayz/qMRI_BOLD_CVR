...
# Linear interpolation between detected EtCO2 peaks
...
print(f"EtCO2 baseline = {etco2_baseline:.2f} mmHg")

# Add interpolated EtCO2 column to DataFrame
...
print(f"Detected {len(etco2_values)} EtCO2 peaks. Mean EtCO2 = {etco2_values.mean():.3f}%")

# Plot for confirmation
plt.figure(figsize=(12, 6))
plt.plot(time, co2, color='blue', linewidth=1, label='PctCO₂ (%)')
plt.plot(etco2_times, etco2_values, color = "red", linewidth=1, label='Detected EtCO2')
plt.plot(time, etco2_interp, 'g--', linewidth=1, label='Interpolated EtCO2')
plt.plot(time, baseline_line, color='magenta', linewidth=2, label='EtCO₂ baseline (slow drift)')

plt.xlabel("Time (sec)")
plt.ylabel("CO2 concentration (%)")
plt.title("Interpolated End-tidal CO2 (EtCO2) Trend")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
